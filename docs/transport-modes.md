...
> | Key | Store | Holds |
> |---|---|---|
> | `lukk:logging-out:<app.baseURL>` | `sessionStorage` | A logout started but not known to have finished: the moment it was asked for, and the session's refresh-token family (`fid`) when one was known. One minute. |
> | `lukk:signed-in-at:<app.baseURL>` | `localStorage` | When a sign-in was last *sent*, in any tab. Only read to decide whether a logout note that names no session is still current. |
>
