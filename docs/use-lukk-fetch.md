...
> [!NOTE]
> **Credentials never leak cross-origin.** The session cookie and bearer are attached only to a **same-origin-as-baseURL** target; a cross-origin URL passed to `useLukkFetch` gets neither. The instance is built with `credentials: 'same-origin'` and *upgrades* to `include` per request, in its `onRequest` hook, once the target is known to be same-origin. A per-call `baseURL` is judged too, not just the path: ofetch applies it **after** the hook runs, so `api('/me', { baseURL: 'https://collector.example' })` would otherwise look like a same-origin `/me` at the moment the decision is made, and leave with your bearer. An absolute per-call base is accepted only on the API's own origin (or this app's, when the API base is the relative BFF mount); a *relative* one resolves against the document, so it is accepted only where the API base is relative too. Deliberately that way round rather than defaulting to `include` and downgrading, because of the caveat below. Also, browsers can't read a manual-redirect target — an upstream `3xx` is surfaced as a navigation on **SSR** but is opaque on the client (the call resolves without following it).

> [!WARNING]
> **A per-call `onRequest` replaces lukk's, it doesn't run alongside it.** ofetch merges per-call options by spreading, so passing your own hook to a single call drops lukk's — the one that sets `Accept`, attaches the bearer or SSR cookie, and performs the same-origin upgrade above. That call keeps the fail-closed `credentials: 'same-origin'` default and sends no lukk credential, which is the safe direction to fail but is rarely what you meant. If you need a per-call hook, call the hook you want from a wrapper around the instance rather than passing `onRequest` through, or create a second instance for that call.
