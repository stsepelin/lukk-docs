...
> [!NOTE]
> **Credentials never leak cross-origin.** The session cookie and bearer are attached only to a **same-origin-as-baseURL** target; a cross-origin URL passed to `useLukkFetch` gets neither. The instance is built with `credentials: 'same-origin'` and *upgrades* to `include` per request, in its `onRequest` hook, once the target is known to be same-origin. Deliberately that way round rather than defaulting to `include` and downgrading, because of the caveat below. Also, browsers can't read a manual-redirect target — an upstream `3xx` is surfaced as a navigation on **SSR** but is opaque on the client (the call resolves without following it).
